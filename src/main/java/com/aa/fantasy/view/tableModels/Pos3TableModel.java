/**
 * Class which handles table models for position 2 (WRs, SFs, 2Bs).
 */
...
public class Pos3TableModel extends AbstractTableModel implements PropertyChangeListener {
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if(evt.getPropertyName().equals("SFs")){  //If the pitchers playing changes
			playerList = (ArrayList<Player>) evt.getNewValue();
			fireTableDataChanged();
		}
		if(evt.getPropertyName().equals("WRs")){  //If the pitchers playing changes
			playerList = (ArrayList<Player>) evt.getNewValue();
			fireTableDataChanged();
		}
		if(evt.getPropertyName().equals("2bs")){  //If the pitchers playing changes
			playerList = (ArrayList<Player>) evt.getNewValue();
			fireTableDataChanged();
		}
	}
