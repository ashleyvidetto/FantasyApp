/**
 * Class which handles table models for position 2 (TEs, PFs, SS).
 */
...
public class Pos4TableModel extends AbstractTableModel implements PropertyChangeListener {
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if(evt.getPropertyName().equals("PFs")){  //If the pitchers playing changes
			playerList = (ArrayList<Player>) evt.getNewValue();
			fireTableDataChanged();
		}
		if(evt.getPropertyName().equals("TEs")){  //If the pitchers playing changes
			playerList = (ArrayList<Player>) evt.getNewValue();
			fireTableDataChanged();
		}
		if(evt.getPropertyName().equals("SS")){  //If the pitchers playing changes
			playerList = (ArrayList<Player>) evt.getNewValue();
			fireTableDataChanged();
		}
	}
