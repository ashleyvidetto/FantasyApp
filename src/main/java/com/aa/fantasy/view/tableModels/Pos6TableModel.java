/**
 * Class which handles table models for position 2 (Catchers, D/ST).
 */
...
public class Pos6TableModel extends AbstractTableModel implements PropertyChangeListener {
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if(evt.getPropertyName().equals("catchers")){  //If the pitchers playing changes
			playerList = (ArrayList<Player>) evt.getNewValue();
			fireTableDataChanged();
		}
		if(evt.getPropertyName().equals("defense")){  //If the pitchers playing changes
			playerList = (ArrayList<Player>) evt.getNewValue();
			fireTableDataChanged();
		}
	}
